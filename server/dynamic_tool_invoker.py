...
from server.dynamic_tools import ResourceContext, get_manager
from server.connection_manager import get_unity_connection_manager

logger = logging.getLogger("dynamic_tool_invoker")

async def invoke_dynamic_tool(tool_name: str, params: Dict[str, Any], ctx: Optional[Context] = None) -> Any:
    """
    ...
    # Use connection manager to handle reconnection if needed
    connection_manager = get_unity_connection_manager()
    
    try:
        async def execute_tool():
            return await client.send_command(tool_name, params)
            
        # Execute with reconnection support
        result = await connection_manager.execute_with_reconnect(execute_tool)
...
async def invoke_dynamic_resource(resource_name: str, params: Dict[str, Any] = None, ctx: Optional[Context] = None) -> Any:
    """
    ...
    normalized_params = _normalize_resource_parameters(resource_name, params)
    ...
    try:
        ...
        # Execute with reconnection support
        result = await connection_manager.execute_with_reconnect(access_resource)
