...
async def get_unity_logs(max_logs: int, ctx: Context) -> List[Dict[str, Any]]:
    """
    Get logs from Unity.
...
def register_unity_logs(mcp: FastMCP) -> None:
    """
    Register the unity_logs resource with the MCP instance.
    
    Args:
        mcp: MCP instance
    """
    @mcp.resource("unity://logs")
    async def unity_logs_resource(max_logs: int = 100, ctx: Context = None) -> List[Dict[str, Any]]:
        """
        Get logs from Unity.
        ...
        Returns:
            List of log entries
        """
        return await get_unity_logs(max_logs, ctx)